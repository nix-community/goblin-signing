...
fn derive_public_key_from_private_key_data(kt: cryptoki::object::KeyType, attributes: Vec<cryptoki::object::Attribute>) -> Result<SupportedPublicKey, String> {
    ...
    match kt {
        ...
        KeyType::EC => {
            let mut ec_point: Vec<u8> = attributes.iter().find_map(|attr| match attr {
                Attribute::EcPoint(point) => Some(point),
                _ => None
            }).ok_or("No EC point in EC key")?.to_vec();
        ...
        _ => todo!("Feel free to implement other key types for your own usage.")
    }
}
