...
use std::{path::{PathBuf, Path}, io::Write, time::Duration};
use clap::{Parser, Subcommand};

use const_oid::AssociatedOid;
use cryptoki::{context::{Pkcs11, CInitializeArgs}, mechanism::Mechanism, object::{Attribute, CertificateType, ObjectHandle, KeyType}, slot::Slot, session::Session};
...
use spki::{SubjectPublicKeyInfoOwned, EncodePublicKey};
use x509_cert::{Certificate, builder::{CertificateBuilder, Builder}, serial_number::SerialNumber, time::Validity};
...
fn initialize_token(pkcs11: &mut Pkcs11, token: &Pkcs11Uri) -> (Slot, Session) {
    println!("Slots available with initialized token: {:?}", pkcs11.get_all_slots().unwrap());
    let slot = token.path_attributes.slot_id.unwrap().try_into().expect("Failed to read the slot ID");
    ...
    let pin = if let Some(pin) = token.query_attributes.pin_value.clone() {
        ...
        prompt_pin()
    }.try_into().expect("Failed to read the PIN value");
    ...
    let _ = pkcs11.init_token(slot, &pin, &token_label).expect("Failed to initialize the token");
    let session = pkcs11.open_rw_session(slot).expect("Failed to open the slot in RW session");

    println!("Session opened in read-write");
    let _ = session.login(cryptoki::session::UserType::So, Some(&pin)).unwrap();
    ...
    let _ = session.login(cryptoki::session::UserType::User, Some(&pin)).unwrap();
...
fn connect_to_token(pkcs11: &mut Pkcs11, token: &Pkcs11Uri) -> (Slot, Session) {
    ...
    let slot = token.path_attributes.slot_id.unwrap().try_into().expect("Failed to read the slot ID");
    let session = pkcs11.open_rw_session(slot).expect("Failed to open the token in RO");
...
fn find_or_create<PKey: EncodePublicKey, S: SessionLike>(session: &Session, signer: &P256Signer<S>, certificate: CertificateParameters, public_key: PKey) -> Result<Certificate, cryptoki_rustcrypto::x509::Error> {
    let cert_template = &[
        ...
        Err(err) => match err {
            cryptoki_rustcrypto::x509::Error::MissingCert => {
                let builder = CertificateBuilder::new(
                    ...
                    certificate.subject,
                    SubjectPublicKeyInfoOwned::from_key(public_key).expect("Failed to encode the public key as SPKI"),
                    signer
                ).expect("Failed to create certificate");

                let cert = builder.build::<ecdsa::der::Signature<p256::NistP256>>().expect("Failed to assemble the certificate");
                cert.pkcs11_store(session, cert_template).expect("Failed to store the certificate");
                Ok(cert)
            },
            _ => Err(err)
        }
...
fn generate_signing_key(session: &Session, key_info: &SigningKeyInfo, mechanism: Mechanism) -> (ObjectHandle, ObjectHandle) {
    ...
    let private_key_template = &[
        ...
        Attribute::Sign(true),
        key_info.label_attribute(),
        key_info.subject_attribute().expect("Failed to DER-encode the subject of the signing key information"),
        Attribute::Sensitive(true)
    ];

    session.generate_key_pair(&mechanism, public_key_template, private_key_template).expect("Failed to generate signing keys")
}
...
fn generate_secrets(mechanism: &str, ca_key_info: SigningKeyInfo, subca_key_info: SigningKeyInfo, ca_parameters: CertificateParameters, subca_parameters: CertificateParameters, session: &Session) -> (Certificate, Certificate) {
    ...
    let (subca_public_key, subca_private_key) = generate_signing_key(&session, &subca_key_info, mechanism);
    ...
    let ca_cert = find_or_create(&session, &ca_signer, ca_parameters,
        ca_signer.verifying_key()).expect("Failed to produce and self-sign the CA certificate");
    // As it is not self-signed, the SPKI is our own public key generated for the occasion.
    let subca_cert = find_or_create(&session, &ca_signer, subca_parameters,
        subca_signer.verifying_key()).expect("Failed to produce and sign the sub CA certificate");

    println!("CA {} and SubCA {} generated or found on the security token: {:?}/{:?} - {:?}/{:?}",
        ca_key_info.subject,
...
fn sign_file<S: SessionLike>(file: &Path, mut parents: Vec<Certificate>, parent_signer: &P256Signer<S>) {
    let contents = std::fs::read(file).expect("Failed to read the PE binary");
    let pe = PE::parse(
            &contents
        ).expect("Failed to parse the PE binary");
    ...
    let issuer;
    {
        ...
        certificate = builder.build::<ecdsa::der::Signature<p256::NistP256>>().expect("Failed to assemble the certificate");
        issuer = cms::signed_data::SignerIdentifier::IssuerAndSerialNumber(
            cms::cert::IssuerAndSerialNumber { issuer: parent.tbs_certificate.issuer.clone(), serial_number: parent.tbs_certificate.serial_number.clone() }
        );

    }
    parents.push(certificate);
    let pe_certificate = create_certificate::<Sha256, _, ecdsa::der::Signature<_>>(&pe, parents, issuer, parent_signer).expect("Failed to produce an PE attribute certificate");
    let mut pe_writer = PEWriter::new(pe).expect("Failed to construct the PE writer");
    pe_writer.attach_certificates(vec![pe_certificate]).expect("Failed to attach a new certificate to PE");
    println!("Signed!");
    std::fs::write(file.with_extension("efi.signed"), pe_writer.write_into().expect("Failed to materialize the new PE")).expect("Failed to write the new PE");
}
...
fn main() {
    let args = Cli::parse();
    let token_uri: Pkcs11Uri = args.token_uri.as_str().try_into().expect("Expected a PKCS#11 URL");
    let module_path = token_uri.query_attributes.module_path.clone().unwrap_or_else(|| std::env::var("PKCS11_SOFTHSM2_MODULE").unwrap());
    ...
    match args.commands {
        ...
        Commands::GenerateSecretMaterial(GenerateSecretMaterialCommand { mechanism, ca_label, subca_label, ca_subject, subca_subject }) => {
            ...
            let subca_signing_key = SigningKeyInfo {
                ...
                subject: subca_subject.parse().expect("Failed to parse SubCA subject"),
            };
            ...
            let subca_parameters = CertificateParameters { label: subca_label, subject: subca_subject.parse().unwrap(), profile:x509_cert::builder::Profile::SubCA { issuer: ca_subject.parse().unwrap(), path_len_constraint: None } };
            let (_ca, _subca) = generate_secrets(&mechanism, ca_signing_key, subca_signing_key, ca_parameters, subca_parameters, &session);
        },
        Commands::SignFile(SignFileCommand { file_to_sign, subca_label, subca_subject, ca_label }) => {
            let (_slot, session) = connect_to_token(&mut pkcs11, &token_uri);

            let cert_template = &[
                Attribute::Label(ca_label.into()),
            ];

            let ca_cert = Certificate::pkcs11_load(&session, cert_template).expect("Failed to locate CA certificate");

            let subca_signing_key = SigningKeyInfo {
                label: subca_label.clone(),
                subject: subca_subject.parse().expect("Failed to parse SubCA subject"),
            };
            let subca_signer = P256Signer::new(&session, subca_signing_key.label.as_bytes()).expect("Failed to obtain Sub CA signer");

            let cert_template = &[
                Attribute::Label(subca_label.into()),
            ];

            let subca_cert = Certificate::pkcs11_load(&session, cert_template).expect("Failed to locate sub CA certificate");
