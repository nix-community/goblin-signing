...
fn certificate_from_efi_database(contents: &[u8]) -> Vec<Certificate> {
    ...
    signature_database.0.into_iter().map(|list| list.signatures.into_iter().filter_map(|signature| signature.certificate).collect::<Vec<_>>()).flatten().collect()
}
...
struct Cli {
    ...
    #[arg(short, long, default_value = "/sys/firmware/efi/efivars/db-d719b2cb-3d3a-4596-a3bc-dad00e67656f")]
...
fn main() {
    ...
    if let Some(db_cert_path) = args.database_certificate {
        certificates.push(
            Certificate::from_der(
                &std::fs::read(db_cert_path).expect("Failed to read the provided database certificate")
    ...
    if let Ok(contents) = std::fs::read(&args.efi_variable_path) {
        ...
        println!("Failed to read the EFI variable path: {}", args.efi_variable_path.display());
    }

    let pe_data = std::fs::read(&args.file_to_verify).expect("Failed to read the PE binary to verify");
    let pe = goblin::pe::PE::parse(&pe_data).expect("Failed to parse the PE binary to verify");

    if certificates.is_empty() {
        println!("Verification without a trust store as no certificate has been given: {:?}", goblin_signing::verify::verify_pe_signatures_no_trust(&pe));
    } else {
        println!("Verification with a trust store as certificates has been given: {:?}", goblin_signing::verify::verify_pe_signatures(&pe, VerificationOptions {
